...
def find_cheats(path):
    ...
    for starting_cell in path:
        filtered_path = list(
            filter(lambda cell: 2 <= heuristic(cell, starting_cell) <= 20, path)
        )

        for jumping_cell in filtered_path:
            if (
                cell_distance_from_end[starting_cell]
                - cell_distance_from_end[jumping_cell]
                - heuristic(starting_cell, jumping_cell)
            ) >= 100:
                possible_cheats += 1
